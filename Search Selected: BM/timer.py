...
for dirpath, dirnames, filenames in os.walk('.'):
    for file in filenames:
        ...
        if file.endswith('search.cpp'):
            ...
            # Compile the C++ file
            subprocess.run(['g++', "BM-Vector-Push/Node.cpp", filepath, '-o', exec_name])
            
            # Time the execution of the program

# Warmup
# Run the C++ program
subprocess.run(["g++", "BM-Vector-Push/Node.cpp", "BM-Vector-Push/search.cpp", "-o", "output"])
...
with open('results - gandhi_half.txt', 'w') as f:
